...
enum RouteMapManager {
    @MainActor
    static func searchLocation(_ modelContext: ModelContext, searchText: String, Region: MKCoordinateRegion?) async {
    ...
    static func removeSearchResults(_ modelContext: ModelContext) {
        let searchPredicate = #Predicate<ToileLocation> { $0.destination == nil }
        try? modelContext.delete(model: ToileLocation.self, where: searchPredicate)
    }
